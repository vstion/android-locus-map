...
public class TarOutputStream
    ...
    public void putNextEntry(TarEntry entry) throws IOException {
        ...
        if ((entry.isUnixTarFormat() && name.length() > TarHeader.NAMELEN) ||
            ...
            throw new InvalidHeaderException("file name '" + name + "' is too long ( " + name.length() + " > " + (entry.isUnixTarFormat()
                    ? TarHeader.NAMELEN
                    : (TarHeader.NAMELEN + TarHeader.PREFIXLEN)) + " bytes )");
        }
    ...
    public void write(byte[] wBuf, int wOffset, int numToWrite)
        ...
        if (this.assemLen > 0) {
            ...
            {
                System.arraycopy(wBuf, wOffset, this.assemBuf,
